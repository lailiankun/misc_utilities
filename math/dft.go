...
func testresponse() {
	...
	b := dft1(f64.FloatToComplex(a[:]))
	for i := range b {
		fmt.Println(i, cmplx.Abs(b[i]), f64.Rad2Deg(cmplx.Phase(b[i])))
	}
}
...
func testphase() {
	...
	for i := 0; i < 10; i++ {
		...
		z := dft1n(x[i:i+N], N)
		for i := 0; i < N; i++ {
			fmt.Println(i, cmplx.Abs(y[i]), cmplx.Abs(z[i]), f64.Rad2Deg(cmplx.Phase(y[i])), f64.Rad2Deg(cmplx.Phase(z[i])))
		}
		fmt.Println()
	}
}
