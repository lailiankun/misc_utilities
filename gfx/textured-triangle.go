...
func reset() {
	cameraPos = f64.Vec3{0, 0, -3}
	cameraFront = f64.Vec3{0, 0, -1}
	cameraUp = f64.Vec3{0, 1, 0}
	modelview.LookAt(cameraPos, cameraPos.Add(cameraFront), cameraUp)
	projection.Perspective(f64.Rad2Deg(40), aspectRatio(), 0.2, 1000)
}
...
func event() {
	for {
		...
		case sdl.MouseWheelEvent:
			cameraPos.Z += f64.Clamp(float64(ev.Y), -0.1, 0.1)
		}
	}
}
...
func blitCubes() {
	Q := f64.Quat{}
	Q = Q.FromAxisAngle(f64.Vec3{0, 1, 0}, rot.Y)
	Ry := Q.Mat4()

	Q = Q.FromAxisAngle(f64.Vec3{1, 0, 0}, rot.X)
	Rx := Q.Mat4()
	...
	for n := 0; n < 10; n++ {
		...
		for _, c := range cube {
			...
			for i := range c.Position {
				c.Position[i] = M.Transform3(c.Position[i])
			}
...
func matMul(m ...*f64.Mat4) f64.Mat4 {
	var p f64.Mat4
	p.Identity()
	for i := range m {
		p.Mul(&p, m[i])
		...
		for x1 < x2 {
			...
			if 0 <= idx && idx < len(zbuffer) && z >= zbuffer[idx] {
				...
				if c.Texture != nil {
					...
					canvas.Set(x1, y1, f64.Vec4{u, v, 1 - u - v, 1})
				}
			}
