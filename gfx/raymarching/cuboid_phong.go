// based on https://www.shadertoy.com/view/MttGz7
package main
...
func initSDL() {
	...
	window.SetTitle("Raymarching Cuboid with Phong Shading")
...
func raymarch() {
	...
	var viewToWorld f64.Mat4
	viewToWorld.LookAt(eye, f64.Vec3{}, f64.Vec3{0, 1, 0})
	...
	for p.Y = 0; p.Y < float64(h); p.Y++ {
		for p.X = 0; p.X < float64(w); p.X++ {
			...
			worldDir := viewToWorld.Transform(raydir(45, r, p))
			dir := f64.Vec3{worldDir.X, worldDir.Y, worldDir.Z}
...
func raydir(fovy float64, size, fragCoord f64.Vec2) f64.Vec4 {
	x := fragCoord.X - size.X/2
	y := fragCoord.Y - size.Y/2
	z := size.Y / math.Tan(fovy*RAD/2)
	return f64.Vec4{x, y, -z, 0}.Normalize()
}
...
func cube(p f64.Vec3) float64 {
	d := p.Abs().Sub(f64.Vec3{1, 1, 1})
	in := math.Min(math.Max(d.X, math.Max(d.Y, d.Z)), 0)
	out := d.MaxScalar(0).Len()
	return in + out
}
