...
/*
   ...
   of the quaternion (cos(alpha), sin(alpha)u) is defined as the quternion q^t := (cos(talpha), sin(talpha) u)
...
func reset() {
	cubes = cubes[:0]
	cubes = append(cubes, newCube(f64.Vec3{0, 0, 0}, 100, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{150, 0, 0}, 60, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{0, 150, 0}, 60, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{0, 0, 150}, 60, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{-150, 0, 0}, 60, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{0, -150, 0}, 60, chroma.RandRGB(), chroma.RandRGB()))
	cubes = append(cubes, newCube(f64.Vec3{0, 0, -150}, 60, chroma.RandRGB(), chroma.RandRGB()))
	...
	goal = -1
	rotating = false
	curq = f64.Quat{0.15, 0.15, 0, math.Sqrt(0.955)}
	deltaq = f64.Quat{0, 0, 0, 1}
	sltotal = 500
	slstart = millis()
	sllast = slstart
		...
		case sdl.MouseMotionEvent:
			if ev.State&(sdl.BUTTON_LMASK|sdl.BUTTON_RMASK) != 0 {
				if goal == -1 {
					...
					deltaq = f64.Quat{cross.X, cross.Y, cross.Z, dot}
					curq = deltaq.Mul(curq)
					rotstart = rotend
...
func setGoal(n int) {
	goal = n
	if 0 <= goal && goal < len(goals) {
		newq = goals[goal].Mul(curq.Inverse())
		if newq.W < 0 {
			newq = newq.Scale(-1.0)
		}
		slstart = millis()
...
func faceNormal(p []f64.Vec3) f64.Vec3 {
	u := p[1].Sub(p[0])
	v := p[2].Sub(p[0])
	N := u.Cross(v)
	N = N.Normalize()
...
func isBackFacing(p []f64.Vec3, u f64.Vec3) bool {
	n := faceNormal(p)
	return n.Dot(u) <= 0
}
 ...
 * convex bodies.
 */
func sortByDist(cubes []*Cube, u f64.Vec3) {
	sort.Slice(cubes, func(i, j int) bool {
		return cubes[i].Pos.Dot(u) < cubes[j].Pos.Dot(u)
	})
}
...
type Cube struct {
	...
	InitialVerts []f64.Vec3
	Verts        []f64.Vec3
	Faces        [][]int
}

func newCube(pos f64.Vec3, width float64, stroke, fill color.RGBA) *Cube {
	c := &Cube{
		Pos:    pos,
		Center: pos,
		Stroke: stroke,
	...
	d := width / 2
	for i := uint(0); i < 8; i++ {
		c.Verts = append(c.Verts, f64.Vec3{
			pos.X + d*c.bit(i, 0),
			pos.Y + d*c.bit(i, 1),
			pos.Z + d*c.bit(i, 2),
		})
	}
	c.InitialVerts = make([]f64.Vec3, len(c.Verts))
...
func (c *Cube) Rotate(q f64.Quat) {
	M := q.Mat4()
	for i := range c.Verts {
		c.Verts[i] = M.Transform3(c.Verts[i])
	}
	c.Pos = M.Transform3(c.Pos)
}
 ...
 */
func (c *Cube) Blit(toScreen, f f64.Vec3) {
...
func (c *Cube) face(i int) []f64.Vec3 {
	p := make([]f64.Vec3, 4)
	for j := range p {
...
func projectPolygons(p [][]f64.Vec3, f f64.Vec3) [][]f64.Vec3 {
	a := make([][]f64.Vec3, len(p))
	for i := range a {
		a[i] = make([]f64.Vec3, len(p[i]))
		for j := range p[i] {
 ...
 */
func project(v, f f64.Vec3) f64.Vec3 {
	return f64.Vec3{
 ...
 */
func blitPolygons(p [][]f64.Vec3, toScreen f64.Vec3, stroke, fill color.RGBA) {
	var M f64.Mat4
	width, height, _ := renderer.OutputSize()
	M.Translate(float64(width)/2, float64(height)/2, 0)

	for i := range p {
		var v []sdl.Point
		if !isBackFacing(p[i], toScreen) {
			for j := range p[i] {
				t := M.Transform3(p[i][j])
