...
func main() {
	...
	for {
		for i := range s {
			s[i].Update()
			fmt.Printf("%d goal %v x %v diff %v control%d\n", n, s[i].goal, s[i].x, s[i].goal-s[i].x, i)
		}
		n++
		fmt.Println()
	}
...
// no control error just goes off to infinity
func (s *State) control0(x, goal float64) float64 {
	return 0
}
